...
const MoreDoctorInfo = ({navigation}) => {
    ...
    useEffect(()=>{
        ...
        firebase.auth().onAuthStateChanged((user)=>{
            if (user) {
                firebase.firestore().collection("H_user").doc(user?.uid.toString() ?? "")
            .onSnapshot((snapshot)=>{
                if (!unmounted) {
...
/>
      ...
      >
          ...
          <TouchableOpacity>
             <View style={{padding:10, backgroundColor:"#eceff1", marginBottom:7, borderRadius:15, width:200, alignItems:"center", justifyContent:"center", flexDirection:"row"}}>
          <Text style={{fontSize:20,}}>Mesaj At &nbsp;&nbsp;&nbsp;
  ...
  <Text style={{marginBottom:7, marginLeft:32, color:"grey"}}>Çalıştığı kurum</Text>
           ...
           <Text style={{fontWeight:"400", fontSize:20, paddingHorizontal:10, flex:1}}>{D_CalisilanYer}</Text>
